...
export const SECTION_META: Record<string, SectionMeta> = {
  ...
  fashionHighlight: { label: 'Fashion Highlight', icon: '✨', desc: 'แฟชั่นไฮไลต์ขนาดใหญ่สไตล์นิตยสาร', hasVisualConfig: true, sourcePath: 'components/sections/FashionSection.tsx', adminPath: '/admin/fashion' },
  fashion:    { label: 'Fashion & Style',         icon: '👗', desc: 'ข้อมูลแฟชั่นรวม ไทม์ไลน์ และแบรนด์', hasVisualConfig: true, sourcePath: 'components/sections/FashionSection.tsx', adminPath: '/admin/fashion' },
...
export const VISUAL_CONFIGS: Record<string, VisualConfigDef> = {
  ...
  profile: {
    theme: {
      label: 'Theme',
      options: [
        { value: 'dark', label: 'Dark (มืด/แสงจัด)', icon: '🌙' },
        { value: 'light', label: 'Light (สว่าง/คลีน)', icon: '☀️' },
      ],
    },
    layout: {
      label: 'Stats Bar',
...
export const DEFAULT_SECTIONS: HomepageSectionsConfig = {
  ...
  profile:    { enabled: true, order: 3, theme: 'dark', layout: 'show' },
  schedule:   { enabled: true, order: 4, layout: 'cards', theme: 'light', limit: 4 },
  content:    { enabled: true, order: 5, limit: 10 },
  fashionHighlight: { enabled: true, order: 6, limit: 6 },
  fashion:    { enabled: true, order: 7, limit: 6 },
  awards:     { enabled: true, order: 8, limit: 6 },
  timeline:   { enabled: true, order: 9, layout: 'alternating', theme: 'default', limit: 5 },
  mediaTags:  { enabled: true, order: 10, layout: 'split', limit: 6, motion: { ...HEAVY_SECTION_MOTION_DEFAULTS.mediaTags! } },
  challenges: { enabled: true, order: 11, layout: 'grid', limit: 3 },
  prizes:     { enabled: true, order: 12, theme: 'default', limit: 3 },
