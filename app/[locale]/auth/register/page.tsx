    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center p-6 bg-[var(--color-bg)]">
        <motion.div 
          ...
          className="bg-surface border border-theme/60 p-16 rounded-[3rem] text-center max-w-sm shadow-2xl relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-nf-gradient" />
          <div className="w-24 h-24 bg-panel rounded-full flex items-center justify-center mx-auto mb-10">
             <PartyPopper className="w-10 h-10 text-accent" />
          </div>
          <h2 className="text-3xl font-display text-primary mb-4 font-light">{t('welcomeHome')} <span className="italic">{t('welcomeHomeHighlight')}</span></h2>
          <p className="text-muted text-sm leading-relaxed mb-10">{t('membershipEstablished')}</p>
          <div className="flex flex-col items-center gap-3">
             <div className="w-6 h-6 rounded-full border-2 border-theme border-t-accent animate-spin" />
             <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-muted opacity-40">{t('redirecting')}</p>
          </div>
        </motion.div>
      </main>
    );
    ...
    <main className="min-h-screen flex flex-col items-center justify-center p-6 bg-[var(--color-bg)] transition-colors duration-500 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-overline text-accent font-bold uppercase tracking-[0.4em] mb-4">{t('membership')}</p>
          <h2 className="text-3xl font-display text-primary leading-tight font-light">{t('createIdentity')} <span className="italic">{t('createIdentityHighlight')}</span></h2>
        </div>
        ...
        >
          ...
          {/* Google OAuth Button */}
          <GoogleLoginButton label={t('joinWithGoogle')} />

          {/* Divider */}
          <div className="flex items-center gap-6 my-10 opacity-40">
            <div className="flex-1 h-px bg-theme" />
            <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-muted">{t('orRegister')}</span>
            <div className="flex-1 h-px bg-theme" />
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[9px] font-bold uppercase tracking-widest text-muted ml-4">{t('displayNameLabel')}</label>
              <div className="relative group/input">
                 <User className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted group-focus-within/input:text-accent transition-colors" />
                 <input
                   type="text"
                   value={displayName}
            ...
            <div className="space-y-2">
              ...
              <p className="text-[9px] text-muted/50 ml-4 flex items-center gap-1.5">
                <ShieldCheck className="w-3 h-3" /> {t('passwordHint')}
              </p>
            </div>

            <div className="space-y-2">
              <label className="text-[9px] font-bold uppercase tracking-widest text-muted ml-4">{t('confirmPasswordLabel')}</label>
              <div className="relative group/input">
                 <Lock className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted group-focus-within/input:text-accent transition-colors" />
                 <input
                   type="password"
        ...
        <div className="mt-12 text-center space-y-6">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted">
            {t('alreadyMember')}{' '}
            <Link href="/auth/login" className="text-accent hover:underline ml-2">
              {t('accessAccount')}
            </Link>
          </p>
