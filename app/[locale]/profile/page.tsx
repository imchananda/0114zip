      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] overflow-hidden">
          ...
          {activeTab === 'profile' && (
            <div>
              {/* Banner */}
              <div className="h-28 bg-gradient-to-r from-[#6cbfd0] to-[#fbdf74]" />

              {/* Avatar */}
              <div className="px-8 -mt-12 flex justify-between items-end">
                <div className="w-24 h-24 rounded-full bg-gradient-to-br from-[#6cbfd0] to-[#fbdf74] flex items-center justify-center text-[#141413] text-3xl font-medium border-4 border-[var(--color-surface)] shadow-lg overflow-hidden z-10 shrink-0">
                  {profile?.avatar_url ? (
                    <img src={profile.avatar_url} alt="" className="w-full h-full object-cover" />
                  ) : (
                    (profile?.display_name || '?')[0].toUpperCase()
