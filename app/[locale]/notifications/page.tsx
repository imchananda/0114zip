      ...
      <>
        ...
        <div className="min-h-screen bg-[var(--color-bg)] pt-24 px-4 pb-12 flex items-center justify-center">
          <div className="text-center">
            ...
            <Link href="/auth/login" className="inline-block mt-3 px-4 py-2 bg-[#6cbfd0] text-[#141413] rounded-lg text-sm">
              เข้าสู่ระบบ
            </Link>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-[var(--color-bg)] pt-24 px-4 pb-12">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-sm text-[var(--color-muted)] mb-6">
            ยังไม่ได้อ่าน: <span className="text-[#6cbfd0] font-medium">{unreadCount}</span>
          </p>
          ...
          <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs transition-all ${
                    filter === key
                      ...
                      : 'bg-[var(--color-surface)] text-[var(--color-muted)] border border-[var(--color-border)] hover:border-[#6cbfd0]/50'
                  }`}
                >
                  {label}
                </button>
            ...
            {unreadCount > 0 && (
              <button onClick={markAllRead} className="text-xs text-[#6cbfd0] hover:underline">
                ✓ อ่านทั้งหมด
              </button>
            )}
          </div>
              ...
              {filtered.map((notif, i) => (
                <motion.div
                  ...
                  className={`bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-4 ${
                    !notif.is_read ? 'border-l-2 border-l-[#6cbfd0]' : ''
                  }`}
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="flex items-center gap-2 mb-0.5">
                            ...
                            {!notif.is_read && (
                              <span className="w-2 h-2 rounded-full bg-[#6cbfd0]" />
                            )}
                          </div>
                      ...
                      <div className="flex gap-2 mt-2">
                        {notif.link && (
                          <Link
                            href={notif.link}
                            className="text-[10px] px-2 py-1 rounded-md bg-[#6cbfd0]/10 text-[#6cbfd0] hover:bg-[#6cbfd0]/20 transition-colors"
                          >
                            ไปดู →
                          </Link>
                        )}
